...
namespace UnityEngine.UI
{
    ...
    /// <summary>
    /// 用于使子RectTransform随可重复使用内容滚动的EventType组件。
    /// </summary>
    /// <remarks>
    /// LoopScrollRect不会自行进行任何裁剪。与Mask组件结合使用，可以将其转换为循环滚动视图。
    /// </remarks>
            ...
            IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        protected float contentSpacing
        {
            get
            {
                ...
                {
                    HorizontalOrVerticalLayoutGroup layout1 = this.m_Content.GetComponent<HorizontalOrVerticalLayoutGroup>();
                    if (layout1 != null)
                    {
        ...
        /// <summary>
        ///     EventType setting for which behavior to use when content moves beyond the confines of its container.
        /// </summary>
        /// <example>
        ///     <code>
        /// using UnityEngine;
        ...
        /// </summary>
        /// <remarks>
        ///     The deceleration rate is the speed reduction per second. EventType value of 0.5 halves the speed each second. The default is 0.135. The
        ...
        public int GetFirstItem(out float offset)
        {
            ...
            {
                float size = this.GetSize(this.m_Content.GetChild(0) as RectTransform, false);
        ...
        public int GetLastItem(out float offset)
        {
            ...
            if (this.itemTypeEnd > this.itemTypeStart)
            {
                ...
                float size = this.GetSize(this.m_Content.GetChild(totalChildCount - idx - 1) as RectTransform, false);
        ...
        public void SrollToCellWithinTime(int index, float time)
        {
            ...
            float dist = 0;
            float offset = 0;
            int currentFirst = this.reverseDirection? this.GetLastItem(out offset) : this.GetFirstItem(out offset);
            ...
            else
            {
                ...
                {
                    float elementSize = (this.GetAbsDimension(this.m_ContentBounds.size) - this.contentSpacing * (this.CurrentLines - 1)) /
                            this.CurrentLines;
        ...
        private IEnumerator ScrollToCellCoroutine(int index, float speed)
        {
            ...
            while (needMoving)
            {
                ...
                if (!this.m_Dragging)
                {
                    ...
                    if (move != 0)
                    {
                        Vector2 offset = this.GetVector(move);
        ...
        public void RefillCellsFromEnd(int endItem = 0, bool alignStart = false)
        {
            ...
            {
                float size = this.reverseDirection? this.NewItemAtEnd(!first) : this.NewItemAtStart(!first);
                if (size < 0)
                {
                    break;
                }
            ...
            while (sizeToFill > sizeFilled)
            {
                float size = this.reverseDirection? this.NewItemAtStart(!first) : this.NewItemAtEnd(!first);
                if (size < 0)
                {
                    break;
                }
        ...
        /// </summary>
        /// <param name="startItem">要填充的第一项</param>
        /// <param name="fillViewRect">当[startItem，totalCount]不足以填满整个视图范围时，我们是否应该向后填充[0，startItem)？ </param>
        /// <param name="contentOffset">第一项相对于视图边界的偏移量</param>
        public void RefillCells(int startItem = 0, bool fillViewRect = false, float contentOffset = 0)
        {
            ...
            float sizeToFill = this.GetAbsDimension(this.viewRect.rect.size) + Mathf.Abs(contentOffset);
            float sizeFilled = 0;
        ...
        public void GetHorizonalOffsetAndSize(out float totalSize, out float offset)
        {
            ...
            {
                float elementSize = (this.m_ContentBounds.size.x - this.contentSpacing * (this.CurrentLines - 1)) / this.CurrentLines;
        ...
        public void GetVerticalOffsetAndSize(out float totalSize, out float offset)
        {
            ...
            {
                float elementSize = (this.m_ContentBounds.size.y - this.contentSpacing * (this.CurrentLines - 1)) / this.CurrentLines;
                totalSize = elementSize * this.TotalLines + this.contentSpacing * (this.TotalLines - 1);
        ...
        internal static Bounds InternalGetBounds(Vector3[] corners, ref Matrix4x4 viewWorldToLocalMatrix)
        {
            ...
            Bounds bounds = new(vMin, Vector3.zero);
