...
namespace ET
{
    ...
    {
        private readonly ConcurrentQueue<NetOperator> mainThreadOperators = new();
        private readonly ConcurrentQueue<NetOperator> netThreadOperators = new();
...
#region 线程安全
        ...
        public ushort GetOpcode(Type type) => this.typeOpcode.GetValueByKey(type);
...
#region 主线程
        ...
        {
            var tcs = new TaskCompletionSource<(uint, uint)>();
            NetOperator netOperator = new() { Op = NetOp.GetChannelConn, ServiceId = serviceId, ChannelId = channelId, Object = tcs };
        ...
        {
            NetOperator netOperator = new() { Op = NetOp.ChangeAddress, ServiceId = serviceId, ChannelId = channelId, Object = ipEndPoint };
            this.netThreadOperators.Enqueue(netOperator);
        }
        ...
        {
            NetOperator netOperator = new() { Op = NetOp.RemoveChannel, ServiceId = serviceId, ChannelId = channelId, ActorId = error };
            this.netThreadOperators.Enqueue(netOperator);
        }
        ...
        public void RegisterAcceptCallback(int serviceId, Action<long, IPEndPoint> action) => this.acceptCallback.Add(serviceId, action);

        public void RegisterReadCallback(int serviceId, Action<long, long, object> action) => this.readCallback.Add(serviceId, action);

        public void RegisterErrorCallback(int serviceId, Action<long, int> action) => this.errorCallback.Add(serviceId, action);
            ...
            while (true)
            {
                ...
                try
                {
                    switch (op.Op)
                    {
                        ...
                        default:
                            throw new($"not found net operator: {op.Op}");
                    }
                }
                catch (Exception e)
                {
...
#region 网络线程
        ...
        {
            NetOperator netOperator = new() { Op = NetOp.OnError, ServiceId = serviceId, ChannelId = channelId, ActorId = error };
            this.mainThreadOperators.Enqueue(netOperator);
        }
