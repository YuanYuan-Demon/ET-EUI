...
namespace ET
{
    public class EventSystem : Singleton<EventSystem>, ISingletonUpdate, ISingletonLateUpdate
    {
        ...
        public async ETTask PublishAsync<Event>(Scene scene, Event a) where Event : struct
        {
            Log.Info($"Scene:[{scene.Name}] 发布异步事件: [{typeof(Event).Name}]");
            List<EventInfo> iEvents;
            if (!this.allEvents.TryGetValue(typeof(Event), out iEvents))
            {
                return;
            }
        ...
        public void Publish<Event>(Scene scene, Event a) where Event : struct
        {
            Log.Info($"Scene:[{scene.Name}] 发布事件: [{typeof(Event).Name}]");
            List<EventInfo> iEvents;
            if (!this.allEvents.TryGetValue(typeof(Event), out iEvents))
            {
                return;
