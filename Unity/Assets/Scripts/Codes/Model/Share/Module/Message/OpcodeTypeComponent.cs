...
namespace ET
{
    ...
    public static class OpcodeTypeComponentSystem
    {
        ...
        {
            protected override void Destroy(OpcodeTypeComponent self) => OpcodeTypeComponent.Instance = null;
        }

        public static bool IsOutrActorMessage(this OpcodeTypeComponent self, ushort opcode) => self.outrActorMessage.Contains(opcode);
        ...
        public static Type GetResponseType(this OpcodeTypeComponent self, Type request)
        {
            if (!self.requestResponse.TryGetValue(request, out var response))
                throw new($"not found response type, request type: {request.GetType().Name}");
            return response;
        }
    }
