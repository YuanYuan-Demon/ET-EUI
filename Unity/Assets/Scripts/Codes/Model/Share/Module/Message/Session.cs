...
namespace ET
{
    ...
    public static class SessionSystem
    {
        ...
        {
            if (!self.requestCallbacks.TryGetValue(response.RpcId, out RpcInfo action))
            {
                return;
            }
            ...
            {
                action.Tcs.SetException(new($"Rpc error, request: {action.Request} response: {response}"));
                return;
            }
        ...
        public static void Send(this Session self, IMessage message) => self.Send(0, message);
        ...
        {
            protected override void Awake(Session self, int name)
            {
                self.ServiceId = name;
