...
namespace ET
{
    ...
    public static class ObjectWaitSystem
    {
        ...
        public static async ETTask<WaitType> Wait<WaitType>(this ObjectWait self, ETCancellationToken cancellationToken = null) where WaitType : struct, IWaitType
        {
            ResultCallback<WaitType> tcs = new ResultCallback<WaitType>();
            Type type = typeof(WaitType);
            self.tcss.Add(type, tcs);
            Log.Info($"等待事件: [{type.Name}]");

            void CancelAction()
            {
                self.Notify(new WaitType() { Error = WaitTypeError.Cancel });
            }
