...
namespace ET.Client
{
    ...
    public static class UIEventComponentSystem
    {
        ...
        {
            self.UIEventHandlers.Clear();
            foreach (var v in EventSystem.Instance.GetTypes(typeof (AUIEventAttribute)))
            {
                var attr = v.GetCustomAttributes(typeof (AUIEventAttribute), false)[0] as AUIEventAttribute;
        ...
        {
            if (self.UIEventHandlers.TryGetValue(windowID, out var handler))
                return handler;
            Log.Error($"windowId : {windowID} is not have any uiEvent");
            return null;
        }

        public static void SetUIClicked(this UIEventComponent self, bool isClicked) => self.IsClicked = isClicked;
    }
}