...
namespace ET.Client
{
    ...
    public static class AnimatorComponentSystem
    {
        ...
        {
            var animator = self.GetParent<Unit>().GetComponent<GameObjectComponent>().GameObject.GetComponentInChildren<Animator>();
        ...
        public static void Update(this AnimatorComponent self)
        {
            ...
            {
                throw new($"动作播放失败: {self.MotionType}", ex);
            }
        }

        public static bool HasParameter(this AnimatorComponent self, string parameter) => self.Parameter.Contains(parameter);
        ...
        public static void PlayInTime(this AnimatorComponent self, MotionType motionType, float time)
        {
            ...
            if (!self.animationClips.TryGetValue(motionType.ToString(), out animationClip))
                throw new($"找不到该动作: {motionType}");

            var motionSpeed = animationClip.length / time;
        ...
        public static void ResetAnimatorSpeed(this AnimatorComponent self) => self.Animator.speed = self.stopSpeed;

        [ObjectSystem]
        public class AnimatorComponentAwakeSystem: AwakeSystem<AnimatorComponent>
        {
            protected override void Awake(AnimatorComponent self) => self.Awake();
        }

        [ObjectSystem]
        public class AnimatorComponentUpdateSystem: UpdateSystem<AnimatorComponent>
        {
            protected override void Update(AnimatorComponent self) => self.Update();
        }
