...
namespace ET.Client
{
    ...
    public static class DlgChatSystem
    {
        ...
        {
            int count = self.ClientScene().GetComponent<ChatComponent>().GetChatMessageCount();
        ...
        public static void OnChatItemLoopHandler(this DlgChat self, Transform transform, int index)
        {
            ...
            ChatInfo chatInfo = self.ClientScene().GetComponent<ChatComponent>().GetChatMessageByIndex(index);
        ...
        public static async ETTask OnClickSend(this DlgChat self)
        {
            try
            {
                int errorCode = await ChatHelper.SendMessage(self.ClientScene(), self.View.EInput_InputMessageTMP_InputField.text);
