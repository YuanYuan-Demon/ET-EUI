...
namespace ET.Client
{
    public static class RouterHelper
    {
        ...
        public static async ETTask<Session> CreateRouterSession(Scene clientScene, IPEndPoint address)
        {
            ...
            if (recvLocalConn == 0)
            {
                throw new($"get router fail: {clientScene.Id} {address}");
            }
        ...
        {
            uint connectId = RandomHelper.RandomUInt32();
            ...
            while (true)
            {
                ...
                if (socket.Available > 0)
                {
                    ...
                    Log.Info(
                        $"router connect finish: {connectId} {recvRemoteConn} {recvLocalConn} {localConn} {remoteConn} {routerAddress} {realAddress}");
