    ...
    public static class PathfindingComponentSystem
    {
        public static void Find(this PathfindingComponent self, float3 start, float3 target, List<float3> result)
        {
            if (self.NavMesh == 0)
            {
                ...
                throw new($"pathfinding ptr is zero: {self.DomainScene().Name}");
            }
        ...
        public static float3 RecastFindNearestPoint(this PathfindingComponent self, float3 pos)
        {
            ...
            {
                throw new(
                    $"RecastFindNearestPoint fail, 可能是位置配置有问题: sceneName:{self.DomainScene().Name} {pos} {self.Name} {self.GetParent<Unit>().Id} {self.GetParent<Unit>().Config.Id} {self.EndPos.ArrayToString()}");
            }

            return new(-self.EndPos[0], self.EndPos[1], self.EndPos[2]);
        }
