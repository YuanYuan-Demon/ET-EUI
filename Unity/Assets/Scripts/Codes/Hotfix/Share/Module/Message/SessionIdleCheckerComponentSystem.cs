...
namespace ET
{
    ...
    {
        protected override void Awake(SessionIdleCheckerComponent self) => self.RepeatedTimer
                = TimerComponent.Instance.NewRepeatedTimer(SessionIdleCheckerComponentSystem.CheckInteral, TimerInvokeType.SessionIdleChecker, self);
    }

    [ObjectSystem]
    public class SessionIdleCheckerComponentDestroySystem: DestroySystem<SessionIdleCheckerComponent>
    {
        protected override void Destroy(SessionIdleCheckerComponent self) => TimerComponent.Instance?.Remove(ref self.RepeatedTimer);
    }
