...
namespace ET.Server
{
    ...
    public static class NetInnerComponentSystem
    {
        ...
        {
            protected override void Awake(NetInnerComponent self, IPEndPoint name)
            {
                NetInnerComponent.Instance = self;
                ...
                switch (self.InnerProtocol)
                {
                    case NetworkProtocol.TCP:
                    {
                        self.ServiceId = NetServices.Instance.AddService(new TService(name, ServiceType.Inner));
                        break;
                    }
                    case NetworkProtocol.KCP:
                    {
                        self.ServiceId = NetServices.Instance.AddService(new KService(name, ServiceType.Inner));
                        break;
                    }
                }
        ...
        {
            protected override void Destroy(NetInnerComponent self) => NetServices.Instance.RemoveService(self.ServiceId);
        }
    }
}