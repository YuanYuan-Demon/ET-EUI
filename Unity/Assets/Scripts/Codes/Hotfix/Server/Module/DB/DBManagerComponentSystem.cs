namespace ET.Server
{
    ...
    public static class DBManagerComponentSystem
    {
        ...
        {
            protected override void Awake(DBManagerComponent self) => DBManagerComponent.Instance = self;
        }

        [ObjectSystem]
        public class DBManagerComponentDestroySystem: DestroySystem<DBManagerComponent>
        {
            protected override void Destroy(DBManagerComponent self) => DBManagerComponent.Instance = null;
        }

        public static DBComponent GetZoneDB(this DBManagerComponent self, int zone)
        {
            var dbComponent = self.DBComponents[zone];
            if (dbComponent != null)
                return dbComponent;

            var startZoneConfig = StartZoneConfigCategory.Instance.Get(zone);
            if (startZoneConfig.DBConnection == "")
                throw new($"zone: {zone} not found mongo connect string");
