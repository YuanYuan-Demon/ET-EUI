...
{
    public static void SpawnLoopItemCode(GameObject gameObject)
    {
    ...
    private static void SpawnCodeForScrollLoopItemBehaviour(GameObject gameObject)
    {
        ...
        strFilePath = Application.dataPath + "/Scripts/Codes/ModelView/Client/Demo/UIItemBehaviour/" + strDlgName + ".Designer.cs";
        var sw = new StreamWriter(strFilePath, false, Encoding.UTF8);
        ...
        strBuilder.Append($"\tpublic partial class Scroll_{strDlgName} : Entity,IAwake,IDestroy,IUIScrollItem \r\n")
                .AppendLine("\t{");
